<?php
/**
 * Prime Home Hub - Fixed Database Setup Script
 */

echo "=== Prime Home Hub Database Setup (Fixed) ===\n\n";
...
try {
    ...
    if ($conn->query($sql)) echo "✓ Shipping info table created\n";
    ...
    if ($conn->query($sql)) echo "✓ Reviews table created\n";
    ...
    foreach ($products as $product) {
        $stmt = $conn->prepare("INSERT INTO products (name, category_id, price, rating, image_url, description, stock) VALUES (?, ?, ?, ?, ?, ?, ?)");
        $stmt->bind_param("sidsssi", $product[0], $product[1], $product[2], $product[3], $product[4], $product[5], $product[6]);
        $stmt->execute();
    }
    ...
    if ($result->num_rows > 0) {
        ...
        $stmt->bind_param("ssssss", $admin_username, $admin_first_name, $admin_last_name, $admin_email, $password_hash, $admin_role);
